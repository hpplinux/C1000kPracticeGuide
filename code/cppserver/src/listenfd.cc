...
int sendbuff(int fd);
int sumaccept = 0;
// listen fd, ÂΩìÊúâÂèØËØª‰∫ã‰ª∂ÂèëÁîüÊó∂Ë∞ÉÁî®
int accept_readfun(int epollfd, int listenfd, timer_link *timerlink) {
    ...
    int acceptindex = 0;
	/**
      π”√pbuffœ»∞— ’µΩµƒfd«Î«Ûª∫¥Ê∆¿¥£¨µ»µΩ“ª∂® ˝¡ø“ª¥Œ–‘∑¢ÀÕ∏¯dispatchœﬂ≥Ã¥¶¿Ì.
	**/
    int *pbuff = (int *)buff;
    ...
    while (true) {
        ...
        if (newacceptfd > 0) {            // accept new fd
            if (newacceptfd >= MAX_FD) {  // Â§™Â§öÈìæÊé• Á≥ªÁªüÊó†Ê≥ïÊé•Âèó
...
/**
order_list[fd].buff ¥Ê¥¢µƒ ≤√¥ ˝æ›ƒÿ?  
∑÷≥…»Ù∏…¥Œ∑¢ÀÕª·≤ªª·µº÷¬“ªÃıµƒ«Î«Û∑÷≥…¡Ω≤ø∑÷ƒÿ?
    ...
    for (;;) {
		...
		***/
		if (g_fd) {
            ...
            /*ª·≤ªª·”–’‚—˘µƒŒ Ã‚£¨∂‡∏ˆ∂º±ªªΩ–—£¨÷ª”–“ª∏ˆµ√µΩ¡Àg_fd,∆‰À˚œﬂ≥Ã
			¥¶¿Ìµƒ ±∫Ú∆‰ µg_fd“—æ≠Œ™null¡À£¨…œ√Ê”–◊‘º∫µƒ µœ÷*/
            pthread_cond_wait(&dispatch_cond, &dispatch_mutex);
            paccept = g_fd;
            g_fd = NULL;
        }
